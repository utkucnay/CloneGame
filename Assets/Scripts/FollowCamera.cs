...
public class FollowCamera : MonoBehaviour
{
    ...
    void Update()
    {
        if (FollowPlayer.transform.position.z >= FollowPlayer.FinishPoint.transform.position.z)
        {
            ...
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(transform.position.x, FollowPlayer.transform.position.y +2, Distance.z + FollowPlayer.transform.position.z), 10f * Time.deltaTime);
            transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(0, 0, 0), 40f * Time.deltaTime);
        }
        else if (FollowPlayer.GetFinish())
        {
            transform.rotation = Quaternion.RotateTowards(transform.rotation,Quaternion.Euler(50,0,0), 40f * Time.deltaTime);
            transform.position = Vector3.MoveTowards(transform.position, new Vector3(transform.position.x, FollowPlayer.transform.position.y + Distance.y + 5, Distance.z + FollowPlayer.transform.position.z),10f * Time.deltaTime) ;
